...
/*   Updated: 2020/10/11 10:26:54 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				Bureaucrat::SignForm( Form * document) const
{
	if (!document)
	{
		...
		<< "> can't sign the form because there is no form" << std::endl;
	}
	else if (this->getGrade() > document->getGradeToBeSigned())
		std::cout << "<" << this->getName() << "> can't sign <"
		<< document->getName() << "> because his grade is too low" << std::endl;
	else if (document->getSigned() == 1)
		std::cout << "<" << this->getName() << "> can't sign <"
		<< document->getName() << "> because the form is already signed" << std::endl;
	else
